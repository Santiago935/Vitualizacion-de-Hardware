...
normalizar_interrogacion_exclamacion()
{
    ...
    # Corregir el cierre según la apertura.
    # ¿...! → ¿...?   /   ¡...? → ¡...!
    ...
    # Agregar apertura cuando hay cierre pero no hay apertura. 
    # Los if solo actúan en líneas sin punto ni coma (línea entera es una sola oración); los sed cubren los casos con separadores.
    
    # Línea entera es pregunta (sin separadores intermedios)
    if [[ "$linea" == *"?"* && "$linea" != *"¿"* && "$linea" != *"¡"* && "$linea" != *"."* && "$linea" != *","* ]]; then
        linea="¿$linea"
    fi
    # Línea entera es exclamación (sin separadores intermedios)
    if [[ "$linea" == *"!"* && "$linea" != *"¡"* && "$linea" != *"¿"* && "$linea" != *"."* && "$linea" != *","* ]]; then
        linea="¡$linea"
    fi
 
    # Pregunta después de punto, sin apertura en ese segmento
    linea=$(echo "$linea" | sed 's/\. \([^¿¡?!][^?]*?\)/. ¿\1/g')
    # Pregunta después de coma, sin apertura en ese segmento
    linea=$(echo "$linea" | sed 's/, \([^¿¡?!][^?]*?\)/, ¿\1/g')
    # Pregunta después de exclamación, sin apertura en ese segmento
    linea=$(echo "$linea" | sed 's/! \([^¿¡?!][^?]*?\)/! ¿\1/g')
 
    # Exclamación después de punto, sin apertura en ese segmento
    # Usa [^.]* para no detenerse en ? intermedios del mismo segmento
    linea=$(echo "$linea" | sed 's/\. \([^¿¡?!][^.]*!\)/. ¡\1/g')
    # Exclamación después de coma, sin apertura en ese segmento
    linea=$(echo "$linea" | sed 's/, \([^¿¡?!][^.]*!\)/, ¡\1/g')
    # Exclamación después de pregunta, sin apertura en ese segmento
    linea=$(echo "$linea" | sed 's/? \([^¿¡?!][^.]*!\)/? ¡\1/g')
...
# Las oraciones con solo ? o solo ! las maneja normalizar_interrogacion_exclamacion.
#   CASO 1: Cierre ?!  →  apertura esperada ¡¿   →  ¡¿texto?!
#   CASO 2: Cierre !?  →  apertura esperada ¿¡   →  ¿¡texto!?
normalizar_signos_combinados()
{
    local linea="$1"

    # BLOQUE 0: apertura doble ya presente (¡¿ o ¿¡) con cierre simple.
    # Se interpreta como intención de doble signo y se completa el cierre.
    # ¡¿texto!  →  ¡¿texto?!     ¡¿texto?  →  ¡¿texto?!
    # ¿¡texto?  →  ¿¡texto!?     ¿¡texto!  →  ¿¡texto!?
    linea=$(echo "$linea" | sed 's/^¡¿\([^?!]*\)!$/¡¿\1?!/g')
    linea=$(echo "$linea" | sed 's/\(\. \)¡¿\([^?!]*\)!/\1¡¿\2?!/g')
    linea=$(echo "$linea" | sed 's/\(? \)¡¿\([^?!]*\)!/\1¡¿\2?!/g')
    linea=$(echo "$linea" | sed 's/\(! \)¡¿\([^?!]*\)!/\1¡¿\2?!/g')
    linea=$(echo "$linea" | sed 's/^¡¿\([^?!]*\)?$/¡¿\1?!/g')
    linea=$(echo "$linea" | sed 's/\(\. \)¡¿\([^?!]*\)?/\1¡¿\2?!/g')
    ...
    # BLOQUE 1: cierre ?!  →  apertura ¡¿
    # [^.?!]* evita cruzar puntos separadores de oraciones
    linea=$(echo "$linea" | sed 's/^\([^¡¿?!][^.?!]*\)?!/¡¿\1?!/g')
    linea=$(echo "$linea" | sed 's/\(\. \)\([^¡¿?!][^.?!]*\)?!/\1¡¿\2?!/g')
    linea=$(echo "$linea" | sed 's/^¿\([^¡¿?!][^.?!]*\)?!/¡¿\1?!/g')
    linea=$(echo "$linea" | sed 's/\(\. \)¿\([^¡¿?!][^.?!]*\)?!/\1¡¿\2?!/g')
...
normalizar_linea()
{
    ...
    # Elimino signos duplicados ANTES de normalizar_puntuacion para que ésta no inserte espacios entre ellos (ej: "??" → "? ?" si se hace después)
    linea=$(eliminar_signos_duplicados "$linea")

    # Protejo cierres combinados ?! y !? para que normalizar_puntuacion no inserte un espacio entre los dos signos (ej: "!?" → "! ?"), lo que
    # rompería luego el patrón de normalizar_signos_combinados
    linea=$(echo "$linea" | sed 's/?!/@@QE@@/g')
    linea=$(echo "$linea" | sed 's/!?/@@EQ@@/g')

    linea=$(normalizar_espacios "$linea")
    linea=$(normalizar_puntuacion "$linea")

    # Restauro puntos suspensivos y cierres combinados antes de analizar preguntas/exclamaciones
    linea=$(echo "$linea" | sed 's/@@@/.../g')
    linea=$(echo "$linea" | sed 's/@@QE@@/?!/g')
    linea=$(echo "$linea" | sed 's/@@EQ@@/!?/g')

    linea=$(normalizar_caracteres "$linea")
    linea=$(normalizar_signos_combinados "$linea")
    linea=$(eliminar_signos_duplicados "$linea")
